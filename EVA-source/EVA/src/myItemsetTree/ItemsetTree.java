...
//			printTree();
	...
	public void buildTreeFromItemsets(ArrayList<Integer>[] itemsetList)
			throws IOException {
		// record start time
		startTimestamp = System.currentTimeMillis();
		...
		// Scan the database to read the transactions
		 // for each line (transaction) until the end of file
		for (int index = 0; index<itemsetList.length; index++) {
			
			if (itemsetList[index].isEmpty())
				continue; 
		
			
		//	int[] itemset =new int[itemsetList[index].size()];
			//itemset = itemsetList[index].toArray(); //.toString().chars().toArray();
			int[] itemset = itemsetList[index].stream().mapToInt(i -> i).toArray();
								
			System.out.println("itemset index "+index);
			for (int i=0; i<itemset.length; i++) {
				System.out.println("itemsets "+ itemset[i]);
