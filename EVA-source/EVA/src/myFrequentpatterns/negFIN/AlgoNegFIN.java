package myFrequentpatterns.negFIN;
...
/*** NOTE: THIS IMPLEMENTATION IS FROM THE SPMF LIBRARY. THE AUTHORS MENTIONED HEREAFTER ARE THE AUTHORS OF THESE ALGORITHM INClUDED IN THE SPMF LIBRARY***/ 
...
public class AlgoNegFIN {
  ...
  public  int outputCount = 0;// number of itemsets found
    ...
    void scanDB(Database database, double minSupport) throws IOException {
    	numOfTrans = 0;
        ...
        for (int i=0; i<database.getTransactions().size(); i++) {
        	line = database.getTransactions().get(i);
            ...
            for (Integer item : line.getItems()) {
            // for each item in the transaction
                // increase the count count of the item by 1
                Integer count = mapItemCount.get(item);
                if (count == null) {
    ...
    void construct_BMC_tree(Database database) throws IOException {
        ...
        for (int i=0; i<database.getTransactions().size(); i++) {
            ...
            if (database.getTransactions().get(i).getItems().isEmpty()) {
                continue;
            }
            ...
            // for each item in the transaction
            int tLen = 0; // tLen
            for (Integer itemInteger: database.getTransactions().get(i).getItems()) {
            
                // get the item
                int itemX = itemInteger;
...
//        nlLenSum += Math.pow(2.0, sameCount) * curNode.nodeset.size();
    ...
    private void updateCount(SetEnumerationTreeNode curNode, int sameCount)
            throws IOException {

        outputCount++;
        // === count all combination that can be made using the node list of
        // this itemset
        if (sameCount > 0) {
            // generate all subsets of the node list except the empty set
            for (long i = 1, max = 1 << sameCount; i < max; i++) {
     ...
     * @throws IOException if error while reading/writting to file
     */
    public void runAlgorithm_memory(Database database, double minsup)
            throws IOException {
        ...
        // create object for writing the output file
        writer = new BufferedWriter(new FileWriter(FileDescriptor.out));
        ...
        // Build BMC-tree
        //construct_BMC_tree(filename);//Lines 2 to 6 of algorithm 3 in the paper
        construct_BMC_tree(database);//Lines 2 to 6 of algorithm 3 in the paper
