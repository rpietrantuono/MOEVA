...
import jmetal.core.AdditionalInfo;
			...
			if(!finished && problemData != null){
				...
				try {
					try {
						...
						//resultFront.additionalInfoSet.setTime(time); 
						//MODIFICA DA RIMUOVERE
						//AdditionalInfo[] additionalInfo = new AdditionalInfo[resultFront.size()]; 
						//for (int i=0; i<resultFront.size();i++) additionalInfo[i] = resultFront.get(i).getAdditionalInfo();
				...
				resultFront.printAdditionalInfoToFile(directory + "/AddInfo." + runs);
				if(runs==0) {
					try {Files.deleteIfExists(Paths.get(directory + "/AddInfoSet"));} catch (IOException e) {e.printStackTrace();}
				}
				resultFront.printAdditionalInfoSetToFile(directory + "/AddInfoSet" );
				
				/*** FOR COMPUTING DISTANCE FOR COP PROBELMS***/
				System.out.println("Compute etst distance called by "+algorithmNameList_[alg]);
				Main.computeTestSetDistance(resultFront,algorithmNameList_[alg], runs);
				Main.seed = Main.seed + 1;
				Main.ran = new Random(Main.seed);
				System.out.println("Seed "+Main.seed+"\n \n");
				Main.writeSolutionsToFile(resultFront,Main.baseDir+"/MyStandardStudyCOP" + "/VAR_"+runs+"_"+algorithmNameList_[alg]); // WRITE INTERNAL SOLUTION TO FILE

			//System.out.println("Size of the resulting front "+resultFront.size());
			System.out.println("Finished "+resultFront.size()+"\n \n");
			System.out.println("run  "+runs);
			DateTimeFormatter dtf = DateTimeFormatter.ofPattern("yyyy/MM/dd HH:mm:ss");  
			   LocalDateTime now = LocalDateTime.now();  
			   System.out.println("Time "+dtf.format(now));  
			System.out.println("finished var "+finished);
				if(!finished){
