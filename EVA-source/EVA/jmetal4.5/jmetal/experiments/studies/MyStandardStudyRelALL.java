 ...
 */
public class MyStandardStudyRelALL extends Experiment {
  ...
  public void algorithmSettings(String problemName, 
    ...
    try {
      ...
      if (!(paretoFrontFile_[problemIndex] == null) && !paretoFrontFile_[problemIndex].equals("")) {
         System.out.println("Pareto Taken "+paretoFrontFile_[0]);
    	  for (int i = 0; i < numberOfAlgorithms; i++)
            parameters[i].put("paretoFrontFile_", paretoFrontFile_[problemIndex]);
          } // if
      
        algorithm[0] = new NSGAII_Settings(problemName).configure(parameters[0]);
  //      algorithm[1] = new IBEA_Settings(problemName).configure(parameters[1]);
    ...
    //    algorithm[3] = new PAES_Settings(problemName).configure(parameters[3]);
        //algorithm[4] = new GDE3_Settings(problemName).configure(parameters[4]);
      } catch (IllegalArgumentException ex) {
      Logger.getLogger(MyStandardStudyRelALL.class.getName()).log(Level.SEVERE, null, ex);
  ...
  public static void main(String[] args) throws JMException, IOException {
    MyStandardStudyRelALL exp = new MyStandardStudyRelALL();

    exp.experimentName_ = "MyStandardStudy";
  //  exp.algorithmNameList_ = new String[]{"NSGAII", "IBEA", "MoCELL", "PAES"};
    exp.algorithmNameList_ = new String[]{"NSGAII"};
   
    exp.problemList_ = new String[]{"ReliabilityProblem90DebugAwareIN"};
 //   exp.paretoFrontFile_ = new String[]{"ReliabilityProblem.rf"};
    exp.paretoFrontFile_ = new String[18] ; // Space allocation for 18 fronts
    
    exp.indicatorList_ = new String[]{"HV", "IGD", "SPREAD"};

    int numberOfAlgorithms = exp.algorithmNameList_.length;

    exp.experimentBaseDirectory_ = "/Users/robertopietrantuono/Documents/RelAlloc90/" +
                                   exp.experimentName_;
 //   exp.paretoFrontDirectory_ = "/Users/robertopietrantuono/Documents/RelAlloc/paretoFront2/";
