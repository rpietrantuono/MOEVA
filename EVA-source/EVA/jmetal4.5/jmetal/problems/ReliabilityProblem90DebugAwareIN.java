...
public class ReliabilityProblem90DebugAwareIN extends Problem  {    
...
//ISTANZA 0
/*
double [][] mTh = {{1/24, 1/24, 1/24, 1/24, 1/24, 1/24, 1/24, 1/24},
...
for (k=0; k< Nist; k++) {
	...
	for (i=0; i<m; i++) {
		...
		/*p.println("\n");
		p.println("valore di mTh");
	
		app = (4 * 25)/100;
		app1 = ( (int)(Math.random() * 10));
		
		if (app1 > 5) app2 = 4 + (app * ( (double)(Math.random() % (double) 1)) );
		else {
			
			app = (4 * 5)/100;
			app2 = 4 - (app * ( (double)(Math.random() % (double) 1)) );
...
// m(d) = omega * [CUMULATIVE distribution]

//numero iniziale totale atteso di difetti


//CASO EXPONENTIAL

/* CUMULATIVE DISTR nel caso di truncated logistic

public double getCumulativeDistributionFunction(double t, double shape, double scale) {
...
double omega= 2167.87;

//m(d) nel caso di TRUNCATED LOGISTIC
public double getMeanValueFunction(double x, double shape, double scale) {
	return omega * getCumulativeDistributionFunction(x, shape, scale);
}

//DERIVATA DI m(d)
public double getFaultDensityFunction(double x, double shape, double scale) {
		return omega * getProbabilityDensityFunction(x, shape, scale);
	}

//DERIVATA DI cumulative distribution nel caso truncated log
public double getProbabilityDensityFunction(double t, double shape, double scale) {
	double y = Math.exp(-(t-scale)/shape);
	return (Math.exp(-t/shape)+y)/(shape*(1.0+y)*(1.0+y));
	
//return	(Math.exp(-t/shape)+ Math.exp(-(t-scale)/shape))/(shape*(1.0+Math.exp(-(t-scale)/shape))*(1.0+Math.exp(-(t-scale)/shape)));
}

//HAZARD RATE (h(t)) nel caso di truncated log

public double hazardRate(double t, double shape, double scale) {
	
	
	//return getFaultDensityFunction(t, shape, scale)/(1.0 - getCumulativeDistributionFunction(t, shape, scale));
	 t= 600;
	 scale = 88.62000115841627;
     shape =  64.43000551193958;
     System.out.println("shape vale "+ shape);
 	 System.out.println("scale vale "+ scale);
	 System.out.println("dentro hazard t vale "+ t);
	 double cdf = (1.0-Math.exp(-t/shape))/(1.0+Math.exp(-(t-scale)/shape));
	 double pdf = (Math.exp(-t/shape)+ Math.exp(-(t-scale)/shape))/(shape*(1.0+Math.exp(-(t-scale)/shape))*(1.0+Math.exp(-(t-scale)/shape)));
	 System.out.println("hazardRate vale "+ pdf/(1-cdf));
	 return pdf/(1-cdf);
	 //return getProbabilityDensityFunction(t, shape, scale)/(1.0 - getCumulativeDistributionFunction(t, shape, scale));
}

//RELIABILITY_k = EXP[-RATE_k*y_k]    
//valore di reliability atteso a y_k assumendo che líhazard rate non cambierÚ dopo il test. 
public double reliabilityK(double t, double shape, double scale) {
	
	double somma = 0.0;
	
	System.out.println("peso prima vale "+ weight[0]);
	System.out.println("peso seconda vale "+ weight[1]);
	scale = 88.62000115841627;
    shape = 64.43000551193958;
    System.out.println("shape vale "+ shape);
	System.out.println("scale vale "+ scale); 	
	System.out.println("per la prima funzionalita' ");
	t= 45;
	System.out.println("ammontare di testing del servizio vale "+ t);
	   
    /*System.out.println("hazardRate vale "+ hazardRate(600.0,shape,scale));
	System.out.println("la rel di k vale "+ Math.exp(-( (hazardRate(600.0,shape,scale)*600.0) )));
	Math.exp(-(hazardRate(t,shape,scale)*t));*/
	
/*	System.out.println("failure rate di k vale "+ getFaultDensityFunction(t, shape, scale));
	somma =  weight[0]* getFaultDensityFunction(t, shape, scale);
	System.out.println("per la seconda funzionalita' ");
	t= 150;
	System.out.println("ammontare di testing del servizio vale "+ t);
	System.out.println("failure rate di k vale "+ getFaultDensityFunction(t, shape, scale));
	somma =  somma + weight[1]* getFaultDensityFunction(t, shape, scale);
	System.out.println("Fail_int_TOT "+ somma);
	
	return  Math.exp(-(getFaultDensityFunction(t, shape, scale) * t));
}

*/
/** mc(t) questa funzione serve per calcolare il numero di bug corretti per un dato campionamento*/
//sarebbe la formula usata nella funzione obiettivo

//Class to evaluate the function mu*exp(mu*s)* md(s)for a given value of x (function 1).
...
public double MonteCarloF1(boolean lconf,  double [][] C, double [] Y) {
	...
	/* STAMPE PER GRAFICO DI E AL VARIARE DELLE RUN
	PrintStream nrunFile=null;
	PrintStream errorFile=null;
	try {
		nrunFile = new PrintStream(new File("/Users/robertopietrantuono/Documents/workspace/jMetal/runFile_"+indexIterationFC+".txt"));
		errorFile =new PrintStream(new File("/Users/robertopietrantuono/Documents/workspace/jMetal/errorFile_"+indexIterationFC+".txt"));
	...
	int previousNRun=0;
	System.out.println("\n\nindexIterationFC "+indexIterationFC);	
	indexIterationFC = indexIterationFC+1;  	
...
//	while (e > 0.00001){
		...
		if (i < h){
			...
			//campioniamo i valori di Thkd
			  ...
			  //  if(k==0) System.out.println("range["+k+"],0: "+range[k][0]);
			   // if(k==0) System.out.println("range["+k+"],1: "+range[k][1]);
			    ...
			    app6=Ex.nextDouble();
			  //  System.out.println("\nDELTA K "+delta[k]);
				//System.out.println("app6 in F1 vale "+ app6);
				//app6=10;
			...
			if (app6 < 0.5){
			   ...
			   //System.out.println("mu[k] vale "+ mu[k]);
		...
		for (t = Perc.size()- h; t < Perc.size(); t++){
			...
			if (aver != 0) {
		  	...
		  	//  System.out.println("aSTDEV  \n"+ stdev);
		  		...
		  		/* PARTE DELLE STAMPE PER ERRORE
		  		Double error = new Double(e);
		  		if(check==true){
		  			nrunFile.println(nRun);
		  			if(error.isNaN())
		  				errorFile.println(0);
		  			else
		  				errorFile.println(e);
...
//	System.out.println("variance  "+vrnTot);
	...
	lconf=true;
//	System.out.println( "l'upper bound for conf BUG: "+ MeanConfidenceInterval(lconf,averTot,vrnTot));
		
	//System.out.println("\n CONFIDENCE BUG "+MeanConfidenceInterval(lconf,averTot,vrnTot/nRun));
	if (nRun > 0) return MeanConfidenceInterval(lconf,averTot,vrnTot); // PASSO IL QUADRATO DELLO STANDARD ERROR
...
public double MonteCarloF2(boolean lconf,  double [][] C, double [] Y) {
			...
			//}
			       ...
			       
				{
				   ...
				   if ((Double.isNaN(app7)) || (app7 == Infinty)) check = false; 
				   		...
				   		else costTot= ((C1 * (app6/24) * app7) + C2 * (app6/24) * (omega[k] - app7) + C3 * Y[k]/24) + costTot;  	
    ...
    */
	...
	lconf=false;
	//restituisco l'intervallo di confidenza di tutte le MC run
//	System.out.println( "il lower bound inter conf COST: "+ MeanConfidenceInterval(lconf,averTot,vrnTot));
 ...
 */
 ...
 public ReliabilityProblem90DebugAwareIN(String solutionType) {
